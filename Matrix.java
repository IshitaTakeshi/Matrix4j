...
public class Matrix {
    ...
    public Matrix add(Matrix another) {
        if(!(this.nRows() == another.nRows() &&
             this.nColumns() == another.nColumns())) {
        ...
        for(int i = 0; i < nRows; i++) {
            sum.setRow(i, this.row(i).add(another.row(i)));
        }
        return sum;
    }

    //this - another = this + (-1) * another
    ...
    public Matrix multiply(double r) {
        ...
        for(int i = 0; i < nRows; i++) {
            product.setRow(i, this.row(i).multiply(r));
        }
        return product;
    }
    ...
    public Matrix multiply(Matrix another) {
        ...
        for(int i = 0; i < nRows; i++) {
            product.setRow(i, this.row(i).multiply(another.row(i)));
        }
        return product;
    }

    //compute matrix multiplication
    public Matrix product(Matrix another) {
        if(this.nColumns() != another.nRows()) {
            throw new IllegalArgumentException();
        }

        int nRows = this.nRows();
        int nColumns = another.nColumns();
        double[][] product = new double[nRows][nColumns];
        for(int i = 0; i < nRows; i++) {
            for(int j = 0; j < nColumns; j++) {
                product[i][j] = this.row(i).product(another.column(j));
            }
        }
        return new Matrix(product);
    }

    public Matrix divide(double r) {
        if(r == 0) {
